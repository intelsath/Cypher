...
module.exports.generate_key = function(mnemonic,passphrase,path){
    if(bip39.validateMnemonic(mnemonic)){
        ...
        var root = new litecore.HDPrivateKey.fromSeed(seed);
        var privateKey = root.derive("m/0/" + path.toString()).privateKey;
        var out = {
            coin: coin_lib,
            //address: privateKey.toAddress().toString(),
			address: privateKey.toAddress(litecore.Networks.testnet).toString(),
            publicKey: new litecore.PublicKey(privateKey).toString(),
