...
module.exports.generate_key = function(mnemonic,passphrase,path){
	if(bip39.validateMnemonic(mnemonic)){
		...
		var hdnode = bitcoin.HDNode.fromSeedBuffer(seed, bitcoin.networks.testnet).derivePath('m/0/' + path.toString()); // 'm/0/0'
		//var hdnode = bitcoin.HDNode.fromSeedBuffer(seed).derivePath('m/0/' + path.toString()); // 'm/0/0'
		var out = {
			coin: coin_lib,
			address: hdnode.getAddress(),
			publicKey: hdnode.keyPair.getPublicKeyBuffer().toString('hex'),
