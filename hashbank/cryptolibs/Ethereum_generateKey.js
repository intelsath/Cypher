...
module.exports.generate_key = function(mnemonic,passphrase,path){
    if(bip39.validateMnemonic(mnemonic)){
        ...
        var root = new hdkey.fromMasterSeed(seed);
        var privateKey = root.derivePath("m/0/" + path.toString())._hdkey._privateKey;
        var privateKeyStr = ethUtil.bufferToHex(privateKey);
        var address = ethUtil.privateToAddress(privateKey);
        var addressStr = ethUtil.bufferToHex(address);
        var publicKey = ethUtil.privateToPublic(privateKey);
        var publicKeyStr = ethUtil.bufferToHex(publicKey);
        var out = {
            coin: coin_lib,
            address: addressStr,
