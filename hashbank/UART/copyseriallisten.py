...
class SerialListen:
    ...
    def send_command(self,k):
        ...
        if k=="3": # Request all addresses
            ...
            if coinname == "All":
                ...
                while self.rx_buffer != 0:
                    ...
                    data_rx = self.uart_rxcommunicate()
                    # wallet_ids.append(data_rx)
                    print "Received wallet id/name: " + data_rx
                print "All Wallet ID AND names received!"

                #json.dumps(wallet_ids)
                #print "All wallet ids received!"

            json.dumps(public_address_list)
            json.dumps(public_key_list)
        ...
        if k=="5":
            ...
            nxt_buffer = self.change_txbuffer(wallet_id)
            self.uart_txcommunicate(nxt_buffer) # Send next buffer
            self.uart_txcommunicate(wallet_id)   # Send coinname
            ...
            try:
                ...
                p1 = subprocess.Popen(cmd_str) # Output in json format
                p1.wait()
